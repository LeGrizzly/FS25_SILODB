-- https://github.com/LeGrizzly/FS25_EmployeeManager
CommandManager = {}
...
function CommandManager:add(name, description, usage, funcName, target)
    if self.commands[name] then
        print("[CommandManager] Command '%s' already registered, updating...", name)
    end
    ...
    print("[CommandManager] Registered command: %s", name)
end
